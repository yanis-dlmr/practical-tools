...
class PictureManager {
    ...
    display_pictures = () => { // Display all the pictures in the output block with their original size
        for (let i = 0; i < this.pictures.length; i++) {
            this.add_output_block('Original picture number ' + (i+1) , 'This is your original picture', this.pictures[i]);
        }
    }
    ...
    compute_average_picture = () => { // Compute the average of all the pictures and display it in the output block, with the original size
        ...
        const sum = this.cv_pictures.reduce((acc, cv_picture) => {
            cv_picture.data.forEach((value, index) => {
                acc[index] += value;
            });
            return acc;
        }, new Array(this.cv_pictures[0].data.length).fill(0));        
        console.log('sum', sum)
    ...
    compute_determine_axis = () => {
        ...
        // process the picture 0
            
        // Find contours
        let src_original = this.cv_pictures[0];
        let src_copy = src_original.clone();
        let src = src_original.clone();
        let dst = cv.Mat.zeros(src.rows, src.cols, cv.CV_8UC3);
        dst.name = 'Contours' + this.cv_pictures[0].name;
        ...
        src.name = 'Threshold' + this.cv_pictures[0].name;
        ...
        dst.name = 'Lines' + this.cv_pictures[0].name;
        this.add_cv_output_block('Lines', text, dst);

        // Draw the lines on the original picture
        const src2 = src_copy;
        src2.name = 'Lines on original picture' + this.cv_pictures[0].name;
        for (let j = 0; j < biggest_contours.length; j++) {
            //const points = biggest_contours[j][0];
            const lefty = biggest_contours[j][1];
            const righty = biggest_contours[j][2];
        ...
        if (light_intensity_bool) {
            ...
            console.log('this.cv_pictures[0].data', this.cv_pictures[0].data);
            for (let j = 0; j < biggest_contours.length; j++) {
                ...
                console.log('this.cv_pictures[0].width', this.cv_pictures[0].width);
                for (let x = 0; x < this.cv_pictures[0].width; x++) {
                    const y = Math.round(a * x + b);
                    if (y < 0 || y >= this.cv_pictures[0].height) {
                        light_intensity_line.push(0);
                    } else {
                        // Get the average light intensity of the 3 channels and don't compute the alpha channel
                        const pixel_r = this.cv_pictures[0].data[(y * this.cv_pictures[0].width + x) * 4 + 0];
                        const pixel_g = this.cv_pictures[0].data[(y * this.cv_pictures[0].width + x) * 4 + 1];
                        const pixel_b = this.cv_pictures[0].data[(y * this.cv_pictures[0].width + x) * 4 + 2];
            ...
            for (let j = 0; j < list_min_max.length; j++) {
                ...
                text += 'The 1st limit is at the index $(' + min[0] + ';' + min[1] + ')$ with the value $' + min[2] + '$\n';
                text += 'The 2nd limit is at the index $(' + max[0] + ';' + max[1] + ')$ with the value $' + max[2] + '$\n';
                ...
                text += 'The pixel length between the limits is $' + pixel_length + '$ $px$\n';
            }
            this.add_cv_output_block('Light intensity limits along lines ', text, src_copy_2);
